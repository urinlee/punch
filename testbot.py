...
async def on_message(message):
    ...
    if message.content.startswith("토큰"):
        userid = message.mentions[0].token
    ...
    if message.content.startswith("반응속도"):
        await asyncio.sleep(0.5)
        await message.channel.send("제가 핑 이라고 할때 먼저 퐁 이라고 하는 사람이 이긴겁니다")
        time = random.randint(1, 10)
        user = message.author.display_name
        await asyncio.sleep(time)
        await message.channel.send(" 핑")
        play = 1
    ...
    if message.content.startswith("내스킨"):
         name=message.content.split(' ')[2]
         response = requests.get('https://ko.namemc.com/search?q=' +str(name))
         readerhtml = response.text
         soup = BeautifulSoup(readerhtml, 'lxml')
         data1 = soup.find('div', class_='col-lg-7')
         data2 = data1.findAll('div', class_='card mb-3')
         data3 = data1.findAll('div', class_='card-header py-0')
         data4 = data1.findAll('div', class_='row align-items-center no-gutters py-2')
         data5 = data1.findAll('div', class_='col-auto d-none d-sm-block')
         data6 = data1.findAll('canvas', class_='skin-2d align-top')
         skin=data6

         await message.channel.send(int(skin))
